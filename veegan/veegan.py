...
from veegan.base import LegacySingleTableBaseline
from veegan.utils import GeneralTransformer, select_device
...
class VEEGAN(LegacySingleTableBaseline):
    ...
    def __init__(
        ...
        self.run["config/gen dim"] = self.gen_dim
        self.run["config/embed dim"] = self.embedding_dim
        self.run["config/recon dim"] = self.rec_dim
        self.run["config/discriminator dim"] = self.dis_dim
        self.run["config/epoch"] = self.epochs



    def fit(self, train_data, categorical_columns=tuple(), ordinal_columns=tuple()):
        print(self.opt.cat_col)
        ...
        for i in range(self.epochs):
            ...
            self.run["output/Recon Loss"].log(loss_r)
